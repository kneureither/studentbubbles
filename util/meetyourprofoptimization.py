...
def solve_meet_prof_optimization(preferences=None, bubble_capacities=None, preference_costs=[0, -1, -2, -10]):
    """
    :param preferences: np.array of size(students, profs) where entries are int numbers between
                0 (no choice for 3rd semester)
                1 (choice for 3rd semester)
                2 (no choice 1st semester)
                3 (choice for 1st semester)
    :param bubble_capacities: list of bubbles and corresponding priorities
    :return: array of size (students, bubbles) with up to two non-zero entry per row
    """
    ...
    student_got_priority = [0 for i in range(len(preference_costs))]
    ...
    for stud_idx, stud_prefs in enumerate(preferences):
        for bubble_idx, bubble_pref in enumerate(stud_prefs):
            ...
            capacities.append(1)
            unit_costs.append(preference_costs[preferences[stud_idx][bubble_idx]])
    ...
    supplies[0] = total_capacity
    supplies[len(supplies) - 1] = -total_capacity

    # overflow edge
    start_nodes.append(0)
    end_nodes.append(num_students + num_bubbles + 1)
    capacities.append(total_capacity)
    unit_costs.append(100)
...
if __name__ == "__main__":
    ...
    bubble_capacities = [5, 5]

    # solve the membership problem
    membership = solve_meet_prof_optimization(preferences, bubble_capacities)
